...
rsync -av --include='*.gz' --exclude='*' --remove-source-files -e "ssh -o StrictHostKeyChecking=no -i $LOG_HOST_KEY" $LOG_DIR/ $LOG_HOST_USER@$LOG_HOST:/data/stats/logs/$HOSTNAME/ &> $RESULTLOG
