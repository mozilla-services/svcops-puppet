...
define marketplace::apps::monolith_aggregator::admin_instance(
    ...
    cron {
        "aggr-${project_dir}":
            ...
            command     => "cd ${installed_dir}/current/monolith-aggregator; ../venv/bin/monolith-extract aggregator.ini --log-level ${log_level} --start-date $(date --date='yesterday' +'%Y-%m-%d') --end-date $(date --date='yesterday' +'%Y-%m-%d') > ${log_file} 2>&1",
            user        => $cron_user,
            hour        => 1,
            minute      => 15;
    }
